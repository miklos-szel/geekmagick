...
class Webserver {
   public:
    ...
    /**
     * @brief Record that a web request was just handled (used to gate GIF autoplay)
     */
    static auto markActivity() -> void;

    /**
     * @brief millis() timestamp of the last handled web request (0 if none yet)
     */
    static auto lastActivityMs() -> uint32_t;

   private:
    ESP8266WebServer _server;
    static uint32_t s_lastActivityMs;
