...
static constexpr size_t GIF_LOG_BUF_SIZE = 96;
...
auto Gif::gifOpenFile(const char* fname, int32_t* pSize) -> void* {
    ...
    if (s_instance->m_fileInUse) {
        Logger::warn((String("gifOpenFile: busy, another file in use, requested ") + path).c_str(), "Gif");
        return nullptr;
    }
    ...
    if (!s_instance->m_file) {
        ...
        Logger::error((String("gifOpenFile: LittleFS.open failed for ") + path).c_str(), "Gif");
...
auto Gif::playOne(const String& path) -> bool {
    if (m_gif == nullptr) {
        if (!begin()) {
            Logger::error((String("playOne: begin() failed for ") + path).c_str(), "Gif");
            return false;
        }
    }
    ...
    if (m_gif->open(path.c_str(), gifOpenFile, gifCloseFile, gifReadFile, gifSeekFile, gifDraw) <= 0) {
        std::array<char, GIF_LOG_BUF_SIZE> errBuf{};
        snprintf(errBuf.data(), errBuf.size(), "playOne: open failed for %s err=%d freeHeap=%u", path.c_str(),
                 m_gif->getLastError(),
                 ESP.getFreeHeap());  // NOLINT(readability-static-accessed-through-instance)
        Logger::error(errBuf.data(), "Gif");
        return false;
    }

    std::array<char, GIF_LOG_BUF_SIZE> okBuf{};
    snprintf(okBuf.data(), okBuf.size(), "playOne: opened %s canvas=%dx%d", path.c_str(), m_gif->getCanvasWidth(),
             m_gif->getCanvasHeight());
    Logger::info(okBuf.data(), "Gif");
    ...
    if (result <= 0) {
        if (m_loopEnabled && !m_stopRequested && !m_currentPath.isEmpty()) {
            ...
            if (m_gif->open(m_currentPath.c_str(), gifOpenFile, gifCloseFile, gifReadFile, gifSeekFile, gifDraw) <= 0) {
                std::array<char, GIF_LOG_BUF_SIZE> errBuf{};
                snprintf(errBuf.data(), errBuf.size(), "update: loop reopen failed for %s err=%d", m_currentPath.c_str(),
                         m_gif->getLastError());
                Logger::error(errBuf.data(), "Gif");
                m_playing = false;
                m_playRequested = false;
